...
public class TeamRoster extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        intent = new Intent(this, UploadData.class);
        myDB = openOrCreateDatabase("FRC", MODE_PRIVATE, null);
        c = myDB.rawQuery("SELECT teamNumber,_id, count(teamNumber) AS matchCount FROM PowerUp GROUP BY teamNumber ORDER BY teamNumber asc", null);
        int i = c.getCount();
        if (i > 0) {
            try {
                teamrosterlistView = (ListView) findViewById(R.id.teamrosterlistView);
